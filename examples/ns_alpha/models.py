...
class LerayAlpha2D(ForwardBVP):
    def __init__(
        ...
        coords,
        Re,
        alpha=0.002  # Add alpha parameter
    ):
        ...
        self.alpha = alpha  # Leray-α parameter
    ...
    def r_net(self, params, t, x, y):
        ...
        # Leray-α terms
        alpha_term_u = self.alpha**2 * ((u_xx + u_yy) * u_x + (v_xx + v_yy) * u_y)
        alpha_term_v = self.alpha**2 * ((u_xx + u_yy) * v_x + (v_xx + v_yy) * v_y)

        # PDE residual for Leray-α equations
        ru = u_t + u * u_x + v * u_y + alpha_term_u + p_x - (u_xx + u_yy) / self.Re  
        rv = v_t + u * v_x + v * v_y + alpha_term_v + p_y - (v_xx + v_yy) / self.Re
        rc = u_x + v_y
