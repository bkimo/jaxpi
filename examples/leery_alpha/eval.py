...
def evaluate(config: ml_collections.ConfigDict, workdir: str):
    ...
    p_pred_list = []
    w_pred_list = []
    U_pred_list = [] # dummy variable?

    # Collect predictions from each time window
    for idx in range(config.training.num_time_windows):
        # Restore the checkpoint for the current time window
        current_ckpt_path = os.path.join('.', "ckpt", config.wandb.name, 'time_window_{}'.format(idx + 1))
        ...
        # The `t_plot_steps` should align with the time interval of the current time window.
        # For simplicity, I'm using the whole `t_ref` here, which assumes `t_ref` covers all time windows continuously.
        # A more robust implementation would filter `t_ref` for each window.
    ...
    save_path = os.path.join(save_dir, "leray_alpha_unsteady_u.pdf")
